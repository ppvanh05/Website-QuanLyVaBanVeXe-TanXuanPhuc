...
export class DieuHanhService implements OnModuleInit {
  ...
  async createRoute(data: any) {
    ...
    return this.prisma.tUYEN_XE.create({
      data: {
        ...
        TrangThai: (data.status === 'locked' || data.status === 'DaKhoa') ? 'DaKhoa' : 'DangHoatDong',
        MaNVDieuPhoi: this.defaultCoordinatorId,
      },
    });
  }
  ...
  async updateRoute(id: string, data: any) {
    ...
    if (data.status !== undefined) updateData.TrangThai = (data.status === 'locked' || data.status === 'DaKhoa') ? 'DaKhoa' : 'DangHoatDong';
  ...
  async createSchedule(data: any) {
    ...
    const newSchedule = await this.prisma.lICH_TRINH.create({
      data: {
        ...
        TrangThai: data.status || 'ChoKhoiHanh',
        MaTuyenXe: routeId,
        MaXe: vehicleId,
        MaNVDieuPhoi: this.defaultCoordinatorId,
      },
